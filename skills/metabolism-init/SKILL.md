...
description: "Initialize knowledge metabolism for a research topic: discuss with user, broad literature survey, build baseline knowledge state"
user-invokable: true
---

# Metabolism Initialization — Day 0 Baseline Building

你正在为一个研究方向执行知识新陈代谢的初始化。这是 Day 0：与用户讨论研究方向，然后构建领域基线知识。

## 准备

1. 检查 `config.json` 是否存在
   - 如果不存在：询问用户研究方向，创建 `config.json`（包含 `keywords`, `arxivCategories`, `currentDay: 0`, `processed_ids: []`）
...
更新 `config.json`：`currentDay` 设为 1。
